  ...
  <head>
	...
	<body class = "body">
		<table>
			...
			<?php
			foreach (get_pref_schools_list() as $item){
				echo '<tr><td>'.$item['mps_id'].'</td>'.
				'<td>'.strtoupper($item['mps_client_ec_no']).'</td>'.
				'<td>'.ucwords(strtolower($item['school_name'])).'</td>';
				//'<td>'.ucwords(strtolower($item['mps_client_level_taught'])).
				//'<td>'.ucwords(strtolower($item['subject1'])).'</td>'.
				//'<td>'.ucwords(strtolower($item['subject2'])).'</td>';
        echo '<td><form method="post" action="preferred_schools_list.php" onsubmit="return confirm(\'Are you sure you want to delete this school?\')">';
        echo '<input type="hidden" value="'.$item['mps_id'].'" name="delete"/>';
