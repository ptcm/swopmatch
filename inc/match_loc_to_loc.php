...
include 'class_lib.php';
      ...
      echo 'Preparing location to location matches'.'<br>';
            
      //declare a variable with the matching conditions
      $conditions = "ON
          mcs.mcs_loc_id = co_prefs.loc_id
          AND mcs.mcs_loc_id != 0
      ...
      //$loc_to_loc reserves all matched records in relative tables 
      //and deactivates the reserved option in other linked tables
      $loc_to_loc = new prep_match_data();
            
      $dup_keys1 = 'mcs_client_ec_no';
      $dup_keys2 = 'EC_NO';
      echo 'The number of unique matches found is '.$loc_to_loc->get_match($matched_records,$dup_keys1, $dup_keys2).'.'.'<br>';
      
      $loc_to_loc->create_matched_recs();

      $loc_to_loc->update_tables();
